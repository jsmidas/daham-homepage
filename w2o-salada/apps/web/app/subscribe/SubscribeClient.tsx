...
type InitialData = {
  ...
  config: { minItems: number; maxItems: number; trialPrice: number };
};

export default function SubscribeClient({ initialData }: { initialData?: InitialData }) {
  return (
    <Suspense fallback={<div className="min-h-screen bg-gradient-to-b from-[#f7fdf9] to-[#edf7f0] flex items-center justify-center"><p className="text-[#7aaa90]">로딩 중...</p></div>}>
      <SubscribeContent initialData={initialData} />
    </Suspense>
  );
}

function SubscribeContent({ initialData }: { initialData?: InitialData }) {
  ...
  const [config, setConfig] = useState(initialData?.config ?? { minItems: 2, maxItems: 10, trialPrice: 6900 });
  ...
  // 설정 + 캘린더 로드 (서버에서 프리페치된 데이터가 없을 때만)
  useEffect(() => {
                      ...
                      return (
                        ...
                        >
                          <div className="h-32 bg-gradient-to-br from-[#e8f5ee] to-[#d4edda] flex items-center justify-center relative overflow-hidden">
                            {p.imageUrl ? (
                              <img src={p.imageUrl} alt={p.name} className="w-full h-full object-cover" width={400} height={128} fetchPriority="high" decoding="async" />
                            ) : (
