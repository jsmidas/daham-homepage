...
export default function DashboardPage() {
  ...
  const statCards = [
    ...
    { label: "결제 대기", value: stats ? `${stats.pendingOrders}건` : "-", sub: "", icon: "payments", color: "bg-amber-500" },
    { label: "준비 중", value: stats ? `${stats.preparingOrders}건` : "-", sub: "", icon: "inventory_2", color: "bg-green-500" },
    { label: "배송 중", value: stats ? `${stats.shippingOrders}건` : "-", sub: "", icon: "local_shipping", color: "bg-purple-500" },
  ];

  const summaryCards = [
    { label: "총 상품", value: stats?.totalProducts ?? 0, icon: "restaurant_menu" },
    { label: "총 회원", value: stats?.totalMembers ?? 0, icon: "people" },
    { label: "활성 구독", value: stats?.activeSubscriptions ?? 0, icon: "autorenew" },
  ];
          ...
          <div key={i} className="bg-white rounded-xl p-5 shadow-sm border">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-800 mt-1">{loading ? "..." : s.value}</p>
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        {summaryCards.map((s, i) => (
          <div key={i} className="bg-white rounded-xl p-5 shadow-sm border text-center">
            ...
            <p className="text-2xl font-bold text-gray-800 mt-2">{loading ? "..." : s.value}</p>
            <p className="text-sm text-gray-500">{s.label}</p>
          </div>
        ))}
      </div>
